<?php
/* ----------------------------------------------------------------------
   $Id: VariationWert.php,v 1.6 2006/07/09 03:29:07 r23 Exp $
...
if (auth())
{
	if (intval($_POST["action"]) == 1 && intval($_POST['KeyEigenschaftWert']))
	{
		...
		//hole einstellungen
		$cur_query = xtc_db_query("SELECT languages_id, tax_class_id, tax_zone_id FROM eazysales_einstellungen");
		...
		if ($products_options_id>0)
		{
			...
			if (!$options_values->products_options_values_id)
			{
				...
				//hole max PK
				$cur_query = xtc_db_query("SELECT max(products_options_values_id) FROM products_options_values");
				$max_id_arr = mysql_fetch_row($cur_query);
				$options_values->products_options_values_id = $max_id_arr[0]+1;
				xtc_db_query("INSERT INTO products_options_values (products_options_values_id,language_id,products_options_values_name) values ($options_values->products_options_values_id,$einstellungen->languages_id,\"$EigenschaftWert->cName\")");			
				
				//erstelle verknpfung zwischen wert und eig
				xtc_db_query("INSERT INTO products_options_values_to_products_options (products_options_id,products_options_values_id) values($products_options_id,$options_values->products_options_values_id)");
			}
			...
			if ($kArtikel>0)
			{
				...
				if ($products_id>0)
				{
					...
					$cur_query = xtc_db_query("SELECT products_tax_class_id FROM products WHERE products_id=".$products_id);
					$cur_tax = mysql_fetch_object($cur_query);
					$Aufpreis = ($EigenschaftWert->fAufpreis/(100+get_tax($cur_tax->products_tax_class_id)))*100;
					xtc_db_query("INSERT INTO products_attributes (products_id,options_id,options_values_id,options_values_price,price_prefix) values($products_id,$products_options_id,$options_values->products_options_values_id,$Aufpreis,\"+\")");
